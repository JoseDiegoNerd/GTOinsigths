-- GTO Insights - Expose imagem_url on the social format views
-- Scope: vw_instagram_metricas_formato / vw_facebook_metricas_formato explicitly enumerate
-- columns, so the new stage_social_format_metrics.imagem_url column added in
-- 20260804_015_thumbnails_and_demographics.sql needs to be added here too, at the end (Postgres
-- forbids reordering columns on CREATE OR REPLACE VIEW).
...
select
  ...
  coalesce(sum(curtidas), 0)::integer as curtidas,
  coalesce(sum(comentarios), 0)::integer as comentarios,
